...
- 每回合結束後方塊下降一格並新增一排。
- 收集青色 `+1`，下一回合會多一顆球。
- 每回合收集金幣，集滿 10 枚可在主畫面的 Ball Lab 抽取永久球款。
- 首批球款包含標準圓球、可穿越窄縫的迷你球與使用三角碰撞外框的稜鏡球。
- 任一方塊抵達底線時遊戲結束。
...
目前版本：`1.11.0 (12)`。專案使用獨立 bundle identifier，以避免側載工具沿用舊 App 容器；每次功能更新都會同步提高 App 的版本或 build number。
